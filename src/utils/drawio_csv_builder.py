...
## ---- CSV below this line. First line are column names. ----\n"""
    ...
    create_target_col_id_references_for_rules(mapping_data_transposed, rules_data_transposed)
    ...
    target : TargetTable = build_target(mapping_data_transposed, target_table_name)
    sources : dict[str, SourceTable] = build_sources(mapping_data_transposed, target_table_name, target)
...
def build_rules(rules_data):
    ...
    rule_id_loc = get_property_location_in_list(rules_data[-1], constants.RULE_ID)
    rule_name_loc = get_property_location_in_list(rules_data[-1], constants.RULE_NAME)
    rule_description_loc = get_property_location_in_list(rules_data[-1], constants.RULE_DESCRIPTION)
    rule_type_loc = get_property_location_in_list(rules_data[-1], constants.RULE_TYPE)
    dependent_rule_id_loc = get_property_location_in_list(rules_data[-1], constants.DEPENDENT_RULE)
    preceeding_rule_id_loc = get_property_location_in_list(rules_data[-1], constants.PRECEEDING_RULE)
    target_refs_loc = get_property_location_in_list(rules_data[-1], constants.TRGS_REFERENCED_BY_RULE)
...
def create_target_col_id_references_for_rules(mapping_data, rules_data):
    ...
    src_rule_refs_loc = get_property_location_in_list(mapping_data[-1], constants.SRC_COL_RULE_REF_VAL)
    trg_id_loc = get_property_location_in_list(mapping_data[-1], constants.TRG_COL_IDENTIFIER)
    rule_id_loc = get_property_location_in_list(rules_data[-1], constants.RULE_ID)
...
def build_sources(mapping_data, target_table_name, target_table: TargetTable):
    ...
    src_sys_num_loc = get_property_location_in_list(mapping_data[-1], constants.SRC_COL_SYC_NUM_HEAD_VAL)
    src_sys_tech_nm_loc = get_property_location_in_list(mapping_data[-1], constants.SRC_SYS_TECH_NAME_VAL)
    src_store_nm_loc = get_property_location_in_list(mapping_data[-1], constants.SRC_STORE_NAME_VAL)
    src_store_desc_loc = get_property_location_in_list(mapping_data[-1], constants.SRC_STORE_DESC_VAL)
    src_col_nm_loc = get_property_location_in_list(mapping_data[-1], constants.SRC_COL_NAME_HEAD_VAL)
    src_col_desc_loc = get_property_location_in_list(mapping_data[-1], constants.SRC_COL_DESC_HEAD_VAL)
    src_lineage_loc = get_property_location_in_list(mapping_data[-1], constants.SRC_COL_LINEAGE_HEAD_VAL)
    src_datatype_loc = get_property_location_in_list(mapping_data[-1], constants.SRC_COL_DATATYPE_HEAD_VAL)
    src_sensitive_flag_loc = get_property_location_in_list(mapping_data[-1], constants.SRC_COL_SENSITIVE_FLAG)
    src_rule_refs_loc = get_property_location_in_list(mapping_data[-1], constants.SRC_COL_RULE_REF_VAL)

    targ_col_id_loc = get_property_location_in_list(mapping_data[-1], constants.TRG_COL_IDENTIFIER)
    ...
    for row in mapping_data[:-1]:
        ...
        if tbl_name is None:
            ...
            target_row = target_table.get_row_by_name_and_lineage(row[src_col_nm_loc], row[src_lineage_loc])
            target_row.add_rule_refs(row[src_rule_refs_loc])
            continue
        elif tbl_name not in table_dict:
            table_dict[tbl_name] = SourceTable(row[src_store_nm_loc], row[src_sys_tech_nm_loc], row[src_store_desc_loc])
...
def build_target(mapping_data, table_name):
    ...
    trg_col_name_loc = get_property_location_in_list(mapping_data[-1], constants.TRG_COL_NAME_VAL)
    trg_col_descr_loc = get_property_location_in_list(mapping_data[-1], constants.TRG_COL_DESCR_VAL)
    trg_col_datatype_loc = get_property_location_in_list(mapping_data[-1], constants.TRG_COL_DATATYPE_VAL)
    trg_col_mode_loc = get_property_location_in_list(mapping_data[-1], constants.TRG_COL_MODE_VAL)
    trg_col_lineage_loc = get_property_location_in_list(mapping_data[-1], constants.TRG_COL_LINEAGE)
    trg_col_id_loc = get_property_location_in_list(mapping_data[-1], constants.TRG_COL_IDENTIFIER)
...
def create_target_lineage(mapping_data):
    ...
    for row in mapping_data[:-1]:
        ...
        if row[datatype_loc] == constants.RECORD:
            ...
            current_lineage = gen_lineage_string(current_lineage, [row[target_name_loc]])
            continue
